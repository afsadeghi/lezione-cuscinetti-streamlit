"""Salvataggio persistente del percorso tramite le funzioni REST di Supabase."""
...
class ProgressStore:
    base_url: str
    api_key: str
    lesson_namespace: str = "cuscinetti-v1"
    timeout: float = 8.0
    ...
    def _storage_code(self, student_code: str) -> str:
        """Separa questa lezione dalle altre senza cambiare il codice visibile."""
        value = f"{self.lesson_namespace}:{student_code}".encode("utf-8")
        return "B-" + hashlib.sha256(value).hexdigest()[:16].upper()

    def load(self, student_code: str) -> dict[str, Any] | None:
        try:
            progress = self._post(
                "get_lesson_progress",
                {"p_student_code": self._storage_code(student_code)},
            )
            ...
            raise ProgressStoreError("Non riesco a collegarmi al database per recuperare il percorso.") from exc
    ...
    def save(self, student_code: str, progress: Mapping[str, Any]) -> None:
        try:
            ...
            raise ProgressStoreError("Non riesco a collegarmi al database per salvare il percorso.") from exc
